...
(defmethod HALF-EXTENTS-WITH-MARGIN ((self BOX-SHAPE))
  (bullet-physics-c++::BOX-SHAPE/GET-HALF-EXTENTS-WITH-MARGIN (ff-pointer self)))

(defmethod HALF-EXTENTS-WITHOUT-MARGIN ((self BOX-SHAPE))
  (bullet-physics-c++::BOX-SHAPE/GET-HALF-EXTENTS-WITHOUT-MARGIN (ff-pointer self)))

(defmethod LOCAL-SUPPORTING-VERTEX ((self BOX-SHAPE) (vec VECTOR3))
  (bullet-physics-c++::BOX-SHAPE/LOCAL-GET-SUPPORTING-VERTEX (ff-pointer self) vec))

(defmethod LOCAL-SUPPORTING-VERTEX-WITHOUT-MARGIN ((self BOX-SHAPE) (vec VECTOR3))
  (bullet-physics-c++::BOX-SHAPE/LOCAL-GET-SUPPORTING-VERTEX-WITHOUT-MARGIN (ff-pointer self) vec))
...
(defmethod BATCHED-UNIT-VECTOR-SUPPORTING-VERTEX-WITHOUT-MARGIN ((self BOX-SHAPE) (vectors VECTOR3) (supportVerticesOut VECTOR3) (numVectors integer))
  (bullet-physics-c++::BOX-SHAPE/BATCHED-UNIT-VECTOR-GET-SUPPORTING-VERTEX-WITHOUT-MARGIN (ff-pointer self) vectors supportVerticesOut numVectors))
  ...
  (setf (slot-value obj 'ff-pointer) 
        (cond
          (box-half-extents ;(check-type box-half-extents integer)
           (bullet-physics-c++::MAKE-BOX-SHAPE (ff-pointer box-half-extents)))
          (t (error 'foo)))))

(defmethod (SETF MARGIN) ( (collisionMargin number) (self BOX-SHAPE))
  (bullet-physics-c++::BOX-SHAPE/SET-MARGIN (ff-pointer self) collisionMargin))

(defmethod (SETF LOCAL-SCALING) ( (scaling VECTOR3) (self BOX-SHAPE))
  (bullet-physics-c++::BOX-SHAPE/SET-LOCAL-SCALING (ff-pointer self) scaling))

(defmethod AABB+ ((self BOX-SHAPE) 
                  ( t-arg1 TRANSFORM)
                  ( aabb-Min VECTOR3)
                  ( aabb-Max VECTOR3))
  (bullet-physics-c++::BOX-SHAPE/GET-AABB (ff-pointer self) t-arg1 aabb-min aabb-max))

(defmethod CALCULATE-LOCAL-INERTIA ((self BOX-SHAPE) (mass number) (inertia VECTOR3))
  (bullet-physics-c++::BOX-SHAPE/CALCULATE-LOCAL-INERTIA (ff-pointer self) mass inertia))

(defmethod PLANE ((self BOX-SHAPE) (planeNormal VECTOR3) (planeSupport VECTOR3) (i integer))
  (bullet-physics-c++::BOX-SHAPE/GET-PLANE (ff-pointer self) planeNormal planeSupport i))

(defmethod NUM-PLANES ((self BOX-SHAPE))
  (bullet-physics-c++::BOX-SHAPE/GET-NUM-PLANES (ff-pointer self)))

(defmethod NUM-VERTICES ((self BOX-SHAPE))
  (bullet-physics-c++::BOX-SHAPE/GET-NUM-VERTICES (ff-pointer self)))

(defmethod NUM-EDGES ((self BOX-SHAPE))
  (bullet-physics-c++::BOX-SHAPE/GET-NUM-EDGES (ff-pointer self)))

(defmethod VERTEX ((self BOX-SHAPE) (i integer) (vtx VECTOR3))
  (bullet-physics-c++::BOX-SHAPE/GET-VERTEX (ff-pointer self) i vtx))

(defmethod PLANE-EQUATION ((self BOX-SHAPE) (plane VECTOR4) (i integer))
  (bullet-physics-c++::BOX-SHAPE/GET-PLANE-EQUATION (ff-pointer self) plane i))

(defmethod EDGE ((self BOX-SHAPE) (i integer) (pa VECTOR3) (pb VECTOR3))
  (bullet-physics-c++::BOX-SHAPE/GET-EDGE (ff-pointer self) i pa pb))

(defmethod INSIDEP ((self BOX-SHAPE) (pt VECTOR3) (tolerance number))
  (bullet-physics-c++::BOX-SHAPE/IS-INSIDE (ff-pointer self) pt tolerance))

(defmethod NAME ((self BOX-SHAPE))
  (bullet-physics-c++::BOX-SHAPE/GET-NAME (ff-pointer self)))

(defmethod NUM-PREFERRED-PENETRATION-DIRECTIONS ((self BOX-SHAPE))
  (bullet-physics-c++::BOX-SHAPE/GET-NUM-PREFERRED-PENETRATION-DIRECTIONS (ff-pointer self)))

(defmethod PREFERRED-PENETRATION-DIRECTION ((self BOX-SHAPE) (index integer) (penetrationVector VECTOR3))
  (bullet-physics-c++::BOX-SHAPE/GET-PREFERRED-PENETRATION-DIRECTION (ff-pointer self) index penetrationVector))
...
(defmethod initialize-instance :after ((obj SPHERE-SHAPE) &key radius)
  ...
  (setf (slot-value obj 'ff-pointer) (bullet-physics-c++::make-sphere-shape 
...
(defmethod initialize-instance :after ((obj CAPSULE-SHAPE) &key radius height)
  ...
  (setf (slot-value obj 'ff-pointer) (bullet-physics-c++::MAKE-CAPSULE-SHAPE radius height)))
...
(defmethod initialize-instance :after ((obj CAPSULE-SHAPE-X) &key radius height)
  (check-type radius number)
  (check-type height number)
  (setf (slot-value obj 'ff-pointer) (bullet-physics-c++::make-capsule-shape-x radius height)))
...
(defmethod initialize-instance :after ((obj CAPSULE-SHAPE-Z) &key radius height)
  (check-type radius number)
  (check-type height number)
  (setf (slot-value obj 'ff-pointer) (bullet-physics-c++::MAKE-CAPSULE-SHAPE-Z radius height)))
...
(defmethod initialize-instance :after ((obj CYLINDER-SHAPE) &key half-extents)
  ...
  (setf (slot-value obj 'ff-pointer) (bullet-physics-c++::make-cylinder-shape (ff-pointer half-extents))))

(defmethod AABB+ ((self CYLINDER-SHAPE) (t-arg1 TRANSFORM) (aabbMin VECTOR3) (aabbMax VECTOR3))
  (bullet-physics-c++::CYLINDER-SHAPE/GET-AABB (ff-pointer self) t-arg1 aabbMin aabbMax))

(defmethod CALCULATE-LOCAL-INERTIA ((self CYLINDER-SHAPE) (mass number) (inertia VECTOR3))
  (bullet-physics-c++::CYLINDER-SHAPE/CALCULATE-LOCAL-INERTIA (ff-pointer self) mass inertia))

(defmethod LOCAL-SUPPORTING-VERTEX-WITHOUT-MARGIN ((self CYLINDER-SHAPE) (vec VECTOR3))
  (bullet-physics-c++::cylinder-shape/local-get-supporting-vertex-without-margin
   (ff-pointer self) vec))
...
(defmethod BATCHED-UNIT-VECTOR-SUPPORTING-VERTEX-WITHOUT-MARGIN ((self CYLINDER-SHAPE) (vectors VECTOR3) (supportVerticesOut VECTOR3) (numVectors integer))
  (bullet-physics-c++::CYLINDER-SHAPE/BATCHED-UNIT-VECTOR-GET-SUPPORTING-VERTEX-WITHOUT-MARGIN (ff-pointer self) vectors supportVerticesOut numVectors))

(defmethod (SETF MARGIN) ( (collisionMargin number) (self CYLINDER-SHAPE))
  (bullet-physics-c++::CYLINDER-SHAPE/SET-MARGIN (ff-pointer self) collisionMargin))

(defmethod LOCAL-SUPPORTING-VERTEX ((self CYLINDER-SHAPE) (vec VECTOR3))
  (bullet-physics-c++::CYLINDER-SHAPE/LOCAL-GET-SUPPORTING-VERTEX (ff-pointer self) vec))

(defmethod UP-AXIS ((self CYLINDER-SHAPE))
  (bullet-physics-c++::CYLINDER-SHAPE/GET-UP-AXIS (ff-pointer self)))

(defmethod ANISOTROPIC-ROLLING-FRICTION-DIRECTION ((self CYLINDER-SHAPE))
  (bullet-physics-c++::CYLINDER-SHAPE/GET-ANISOTROPIC-ROLLING-FRICTION-DIRECTION (ff-pointer self)))

(defmethod RADIUS ((self CYLINDER-SHAPE))
  (bullet-physics-c++::CYLINDER-SHAPE/GET-RADIUS (ff-pointer self)))

(defmethod (SETF LOCAL-SCALING) ( (scaling VECTOR3) (self CYLINDER-SHAPE))
  (bullet-physics-c++::CYLINDER-SHAPE/SET-LOCAL-SCALING (ff-pointer self) scaling))

(defmethod NAME ((self CYLINDER-SHAPE))
  (bullet-physics-c++::CYLINDER-SHAPE/GET-NAME (ff-pointer self)))

(defmethod CALCULATE-SERIALIZE-BUFFER-SIZE ((self CYLINDER-SHAPE))
  (bullet-physics-c++::CYLINDER-SHAPE/CALCULATE-SERIALIZE-BUFFER-SIZE (ff-pointer self)))

(defmethod ->serial ((self CYLINDER-SHAPE) &key data-buffer serializer &allow-other-keys)
  (bullet-physics-c++::CYLINDER-SHAPE/SERIALIZE (ff-pointer self) data-buffer serializer))
...
(defmethod initialize-instance :after ((obj CYLINDER-SHAPE-X) &key half-extents)
  (check-type half-Extents VECTOR3)
  (setf (slot-value obj 'ff-pointer) (bullet-physics-c++::MAKE-CYLINDER-SHAPE-X (ff-pointer half-extents))))

(defmethod LOCAL-SUPPORTING-VERTEX-WITHOUT-MARGIN ((self CYLINDER-SHAPE-X) (vec VECTOR3))
  (bullet-physics-c++::CYLINDER-SHAPE-X/LOCAL-GET-SUPPORTING-VERTEX-WITHOUT-MARGIN (ff-pointer self) vec))
...
(defmethod BATCHED-UNIT-VECTOR-SUPPORTING-VERTEX-WITHOUT-MARGIN ((self CYLINDER-SHAPE-X) (vectors VECTOR3) (supportVerticesOut VECTOR3) (numVectors integer))
  (bullet-physics-c++::CYLINDER-SHAPE-X/BATCHED-UNIT-VECTOR-GET-SUPPORTING-VERTEX-WITHOUT-MARGIN (ff-pointer self) vectors supportVerticesOut numVectors))

(defmethod NAME ((self CYLINDER-SHAPE-X))
  (bullet-physics-c++::CYLINDER-SHAPE-X/GET-NAME (ff-pointer self)))

(defmethod RADIUS ((self CYLINDER-SHAPE-X))
  (bullet-physics-c++::CYLINDER-SHAPE-X/GET-RADIUS (ff-pointer self)))
...
(defmethod initialize-instance :after ((obj CYLINDER-SHAPE-Z) &key half-extents)
  (check-type half-Extents VECTOR3)
  (setf (slot-value obj 'ff-pointer) (bullet-physics-c++::MAKE-CYLINDER-SHAPE-Z (ff-pointer half-Extents))))

(defmethod LOCAL-SUPPORTING-VERTEX-WITHOUT-MARGIN ((self CYLINDER-SHAPE-Z) (vec VECTOR3))
  (bullet-physics-c++::CYLINDER-SHAPE-Z/LOCAL-GET-SUPPORTING-VERTEX-WITHOUT-MARGIN (ff-pointer self) vec))
...
(defmethod BATCHED-UNIT-VECTOR-SUPPORTING-VERTEX-WITHOUT-MARGIN ((self CYLINDER-SHAPE-Z) (vectors VECTOR3) (supportVerticesOut VECTOR3) (numVectors integer))
  (bullet-physics-c++::CYLINDER-SHAPE-Z/BATCHED-UNIT-VECTOR-GET-SUPPORTING-VERTEX-WITHOUT-MARGIN (ff-pointer self) vectors supportVerticesOut numVectors))

(defmethod NAME ((self CYLINDER-SHAPE-Z))
  (bullet-physics-c++::CYLINDER-SHAPE-Z/GET-NAME (ff-pointer self)))

(defmethod RADIUS ((self CYLINDER-SHAPE-Z))
  (bullet-physics-c++::CYLINDER-SHAPE-Z/GET-RADIUS (ff-pointer self)))
...
(defmethod initialize-instance :after ((obj CONE-SHAPE) &key radius height)
  ...
  (setf (slot-value obj 'ff-pointer) (bullet-physics-c++::MAKE-CONE-SHAPE radius height)))
...
(defmethod BATCHED-UNIT-VECTOR-SUPPORTING-VERTEX-WITHOUT-MARGIN ((self CONE-SHAPE) (vectors VECTOR3) (supportVerticesOut VECTOR3) (numVectors integer))
  (bullet-physics-c++::CONE-SHAPE/BATCHED-UNIT-VECTOR-GET-SUPPORTING-VERTEX-WITHOUT-MARGIN (ff-pointer self) vectors supportVerticesOut numVectors))

(defmethod RADIUS ((self CONE-SHAPE))
  (bullet-physics-c++::CONE-SHAPE/GET-RADIUS (ff-pointer self)))

(defmethod HEIGHT ((self CONE-SHAPE))
  (bullet-physics-c++::CONE-SHAPE/GET-HEIGHT (ff-pointer self)))

(defmethod CALCULATE-LOCAL-INERTIA ((self CONE-SHAPE) (mass number) (inertia VECTOR3))
  (bullet-physics-c++::CONE-SHAPE/CALCULATE-LOCAL-INERTIA (ff-pointer self) mass inertia))

(defmethod NAME ((self CONE-SHAPE))
  (bullet-physics-c++::CONE-SHAPE/GET-NAME (ff-pointer self)))

(defmethod (SETF CONE-UP-INDEX) ( (upIndex integer) (self CONE-SHAPE))
  (bullet-physics-c++::CONE-SHAPE/SET-CONE-UP-INDEX (ff-pointer self) upIndex))

(defmethod CONE-UP-INDEX ((self CONE-SHAPE))
  (bullet-physics-c++::CONE-SHAPE/GET-CONE-UP-INDEX (ff-pointer self)))

(defmethod ANISOTROPIC-ROLLING-FRICTION-DIRECTION ((self CONE-SHAPE))
  (bullet-physics-c++::CONE-SHAPE/GET-ANISOTROPIC-ROLLING-FRICTION-DIRECTION (ff-pointer self)))

(defmethod (SETF LOCAL-SCALING) ( (scaling VECTOR3) (self CONE-SHAPE))
  (bullet-physics-c++::CONE-SHAPE/SET-LOCAL-SCALING (ff-pointer self) scaling))

(defmethod CALCULATE-SERIALIZE-BUFFER-SIZE ((self CONE-SHAPE))
  (bullet-physics-c++::CONE-SHAPE/CALCULATE-SERIALIZE-BUFFER-SIZE (ff-pointer self)))
...
(defmethod ANISOTROPIC-ROLLING-FRICTION-DIRECTION ((self CONE-SHAPE-X))
  (bullet-physics-c++::CONE-SHAPE-X/GET-ANISOTROPIC-ROLLING-FRICTION-DIRECTION (ff-pointer self)))

(defmethod NAME ((self CONE-SHAPE-X))
  (bullet-physics-c++::CONE-SHAPE-X/GET-NAME (ff-pointer self)))

(defmethod initialize-instance :after ((obj CONE-SHAPE-Z) &key radius height)
  (check-type radius number) (check-type height number)
  (setf (slot-value obj 'ff-pointer) (bullet-physics-c++::MAKE-CONE-SHAPE-Z radius height)))

(defmethod ANISOTROPIC-ROLLING-FRICTION-DIRECTION ((self CONE-SHAPE-Z))
  (bullet-physics-c++::CONE-SHAPE-Z/GET-ANISOTROPIC-ROLLING-FRICTION-DIRECTION (ff-pointer self)))

(defmethod NAME ((self CONE-SHAPE-Z))
  (bullet-physics-c++::CONE-SHAPE-Z/GET-NAME (ff-pointer self)))
...
(defmethod initialize-instance :after ((obj static-plane-shape) &key plane-normal plane-constant)
  ...
  (setf (ff-pointer obj)
        (bullet-physics-c++::make-static-plane-shape (ff-pointer plane-normal) 
                                 (coerce plane-constant 'double-float))))

(defmethod AABB+ ((self STATIC-PLANE-SHAPE) (t-arg1 TRANSFORM) (aabbMin VECTOR3) (aabbMax VECTOR3))
  (bullet-physics-c++::STATIC-PLANE-SHAPE/GET-AABB (ff-pointer self) t-arg1 aabbMin aabbMax))

(defmethod PROCESS-ALL-TRIANGLES ((self STATIC-PLANE-SHAPE) callback (aabbMin VECTOR3) (aabbMax VECTOR3))
  (bullet-physics-c++::STATIC-PLANE-SHAPE/PROCESS-ALL-TRIANGLES (ff-pointer self) callback aabbMin aabbMax))

(defmethod CALCULATE-LOCAL-INERTIA ((self STATIC-PLANE-SHAPE) (mass number) (inertia VECTOR3))
  (bullet-physics-c++::STATIC-PLANE-SHAPE/CALCULATE-LOCAL-INERTIA (ff-pointer self) mass inertia))

(defmethod (SETF LOCAL-SCALING) ( (scaling VECTOR3) (self STATIC-PLANE-SHAPE))
  (bullet-physics-c++::STATIC-PLANE-SHAPE/SET-LOCAL-SCALING (ff-pointer self) scaling))

(defmethod LOCAL-SCALING ((self STATIC-PLANE-SHAPE))
  (bullet-physics-c++::STATIC-PLANE-SHAPE/GET-LOCAL-SCALING (ff-pointer self)))

(defmethod PLANE-NORMAL ((self STATIC-PLANE-SHAPE))
  (bullet-physics-c++::STATIC-PLANE-SHAPE/GET-PLANE-NORMAL (ff-pointer self)))

(defmethod PLANE-CONSTANT ((self STATIC-PLANE-SHAPE))
  (bullet-physics-c++::STATIC-PLANE-SHAPE/GET-PLANE-CONSTANT (ff-pointer self)))

(defmethod NAME ((self STATIC-PLANE-SHAPE))
  (bullet-physics-c++::STATIC-PLANE-SHAPE/GET-NAME (ff-pointer self)))

(defmethod CALCULATE-SERIALIZE-BUFFER-SIZE ((self STATIC-PLANE-SHAPE))
  (bullet-physics-c++::STATIC-PLANE-SHAPE/CALCULATE-SERIALIZE-BUFFER-SIZE (ff-pointer self)))
  ...
  (setf (slot-value obj 'ff-pointer) 
        (cond
          ((and points num-points stride)
           (bullet-physics-c++::MAKE-CONVEX-HULL-SHAPE/with-points&num-points&stride points num-Points stride))
          ((and points num-points)
           (bullet-physics-c++::MAKE-CONVEX-HULL-SHAPE/with-points&num-points points num-Points))
          (points
           (bullet-physics-c++::MAKE-CONVEX-HULL-SHAPE/with-points points))
          (t (MAKE-CONVEX-HULL-SHAPE)))))

(defmethod ADD-POINT ((self CONVEX-HULL-SHAPE) (point VECTOR3)
                      &key (recalculate-local-aabb-p t calc?))
  (if calc?
      (bullet-physics-c++::CONVEX-HULL-SHAPE/ADD-POINT/with-recalculate-local-aabb
       (ff-pointer self) point recalculate-local-aabb-p)
      (bullet-physics-c++::CONVEX-HULL-SHAPE/ADD-POINT (ff-pointer self) point)))

(defmethod UNSCALED-POINTS ((self CONVEX-HULL-SHAPE))
  (bullet-physics-c++::CONVEX-HULL-SHAPE/GET-UNSCALED-POINTS (ff-pointer self)))

(defmethod UNSCALED-POINTS ((self CONVEX-HULL-SHAPE))
  (bullet-physics-c++::CONVEX-HULL-SHAPE/GET-UNSCALED-POINTS (ff-pointer self)))

(defmethod POINTS ((self CONVEX-HULL-SHAPE))
  (bullet-physics-c++::CONVEX-HULL-SHAPE/GET-POINTS (ff-pointer self)))

(defmethod SCALED-POINT ((self CONVEX-HULL-SHAPE) (i integer))
  (bullet-physics-c++::CONVEX-HULL-SHAPE/GET-SCALED-POINT (ff-pointer self) i))

(defmethod NUM-POINTS ((self CONVEX-HULL-SHAPE))
  (bullet-physics-c++::CONVEX-HULL-SHAPE/GET-NUM-POINTS (ff-pointer self)))

(defmethod LOCAL-SUPPORTING-VERTEX ((self CONVEX-HULL-SHAPE) (vec VECTOR3))
  (bullet-physics-c++::CONVEX-HULL-SHAPE/LOCAL-GET-SUPPORTING-VERTEX (ff-pointer self) vec))

(defmethod LOCAL-SUPPORTING-VERTEX-WITHOUT-MARGIN ((self CONVEX-HULL-SHAPE) (vec VECTOR3))
  (bullet-physics-c++::CONVEX-HULL-SHAPE/LOCAL-GET-SUPPORTING-VERTEX-WITHOUT-MARGIN (ff-pointer self) vec))
...
(defmethod BATCHED-UNIT-VECTOR-SUPPORTING-VERTEX-WITHOUT-MARGIN ((self CONVEX-HULL-SHAPE) (vectors VECTOR3) (supportVerticesOut VECTOR3) (numVectors integer))
  (bullet-physics-c++::CONVEX-HULL-SHAPE/BATCHED-UNIT-VECTOR-GET-SUPPORTING-VERTEX-WITHOUT-MARGIN (ff-pointer self) vectors supportVerticesOut numVectors))
...
(defmethod PROJECT ((self CONVEX-HULL-SHAPE) (trans TRANSFORM) (dir VECTOR3) minProj maxProj (witnesPtMin VECTOR3) (witnesPtMax VECTOR3))
  (bullet-physics-c++::CONVEX-HULL-SHAPE/PROJECT (ff-pointer self) trans dir minProj maxProj witnesPtMin witnesPtMax))

(defmethod NAME ((self CONVEX-HULL-SHAPE))
  (bullet-physics-c++::CONVEX-HULL-SHAPE/GET-NAME (ff-pointer self)))

(defmethod NUM-VERTICES ((self CONVEX-HULL-SHAPE))
  (bullet-physics-c++::CONVEX-HULL-SHAPE/GET-NUM-VERTICES (ff-pointer self)))
...
(defmethod PLANE ((self CONVEX-HULL-SHAPE) (planeNormal VECTOR3) (planeSupport VECTOR3) (i integer))
  (bullet-physics-c++::CONVEX-HULL-SHAPE/GET-PLANE (ff-pointer self) planeNormal planeSupport i))

(defmethod INSIDEP ((self CONVEX-HULL-SHAPE) (pt VECTOR3) (tolerance number))
  (bullet-physics-c++::CONVEX-HULL-SHAPE/IS-INSIDE (ff-pointer self) pt tolerance))

(defmethod (SETF LOCAL-SCALING) ( (scaling VECTOR3) (self CONVEX-HULL-SHAPE))
  (bullet-physics-c++::CONVEX-HULL-SHAPE/SET-LOCAL-SCALING (ff-pointer self) scaling))

(defmethod CALCULATE-SERIALIZE-BUFFER-SIZE ((self CONVEX-HULL-SHAPE))
  (bullet-physics-c++::CONVEX-HULL-SHAPE/CALCULATE-SERIALIZE-BUFFER-SIZE (ff-pointer self)))

(defmethod ->serial ((self CONVEX-HULL-SHAPE) &key data-buffer serializer &allow-other-keys)
  (bullet-physics-c++::CONVEX-HULL-SHAPE/SERIALIZE (ff-pointer self) data-buffer serializer))

(defmethod (setf WELDING-THRESHOLD) ( (obj TRIANGLE-MESH) arg0)
  (bullet-physics-c++::TRIANGLE-MESH/WELDING-THRESHOLD/SET (ff-pointer obj) arg0))

(defmethod WELDING-THRESHOLD ((obj TRIANGLE-MESH))
  (bullet-physics-c++::TRIANGLE-MESH/WELDING-THRESHOLD/GET (ff-pointer obj)))
...
(defmethod initialize-instance :after ((obj TRIANGLE-MESH)
  ...
  (setf (slot-value obj 'ff-pointer)
        (cond ((and bits? components?)
               (bullet-physics-c++::MAKE-TRIANGLE-MESH/with-use-32-bit-indices&use-4-component-vertices
...
(defmethod USE-32BIT-INDICES ((self TRIANGLE-MESH))
  (bullet-physics-c++::TRIANGLE-MESH/GET-USE-32BIT-INDICES (ff-pointer self)))

(defmethod USE-4COMPONENT-VERTICES ((self TRIANGLE-MESH))
  (bullet-physics-c++::TRIANGLE-MESH/GET-USE-4COMPONENT-VERTICES (ff-pointer self)))
...
(defmethod initialize-instance :after ((obj CONVEX-TRIANGLE-MESH-SHAPE)
  ...
  (setf (slot-value obj 'ff-pointer) 
        (if calc? (bullet-physics-c++::MAKE-CONVEX-TRIANGLE-MESH-SHAPE/with-calc-aabb meshInterface calcAabb)
            (bullet-physics-c++::MAKE-CONVEX-TRIANGLE-MESH-SHAPE meshInterface))))

(defmethod MESH-INTERFACE ((self CONVEX-TRIANGLE-MESH-SHAPE))
  (bullet-physics-c++::CONVEX-TRIANGLE-MESH-SHAPE/GET-MESH-INTERFACE (ff-pointer self)))

(defmethod LOCAL-SUPPORTING-VERTEX ((self CONVEX-TRIANGLE-MESH-SHAPE)
                                    (vec VECTOR3))
  (bullet-physics-c++::CONVEX-TRIANGLE-MESH-SHAPE/LOCAL-GET-SUPPORTING-VERTEX (ff-pointer self) vec))

(defmethod LOCAL-SUPPORTING-VERTEX-WITHOUT-MARGIN
    ((self CONVEX-TRIANGLE-MESH-SHAPE) (vec VECTOR3))
  (bullet-physics-c++::CONVEX-TRIANGLE-MESH-SHAPE/LOCAL-GET-SUPPORTING-VERTEX-WITHOUT-MARGIN
   (ff-pointer self) vec))
...
(defmethod BATCHED-UNIT-VECTOR-SUPPORTING-VERTEX-WITHOUT-MARGIN 
  ...
  (bullet-physics-c++::CONVEX-TRIANGLE-MESH-SHAPE/BATCHED-UNIT-VECTOR-GET-SUPPORTING-VERTEX-WITHOUT-MARGIN 
   (ff-pointer self) vectors supportVerticesOut numVectors))

(defmethod NAME ((self CONVEX-TRIANGLE-MESH-SHAPE))
  (bullet-physics-c++::CONVEX-TRIANGLE-MESH-SHAPE/GET-NAME (ff-pointer self)))

(defmethod NUM-VERTICES ((self CONVEX-TRIANGLE-MESH-SHAPE))
  (bullet-physics-c++::CONVEX-TRIANGLE-MESH-SHAPE/GET-NUM-VERTICES (ff-pointer self)))

(defmethod NUM-EDGES ((self CONVEX-TRIANGLE-MESH-SHAPE))
  (bullet-physics-c++::CONVEX-TRIANGLE-MESH-SHAPE/GET-NUM-EDGES (ff-pointer self)))
...
(defmethod PLANE ((self CONVEX-TRIANGLE-MESH-SHAPE) (planeNormal VECTOR3) (planeSupport VECTOR3) (i integer))
  (bullet-physics-c++::CONVEX-TRIANGLE-MESH-SHAPE/GET-PLANE (ff-pointer self) planeNormal planeSupport i))

(defmethod INSIDEP ((self CONVEX-TRIANGLE-MESH-SHAPE) (pt VECTOR3) (tolerance number))
  (bullet-physics-c++::CONVEX-TRIANGLE-MESH-SHAPE/IS-INSIDE (ff-pointer self) pt tolerance))

(defmethod (SETF LOCAL-SCALING) ( (scaling VECTOR3) (self CONVEX-TRIANGLE-MESH-SHAPE))
  (bullet-physics-c++::CONVEX-TRIANGLE-MESH-SHAPE/SET-LOCAL-SCALING (ff-pointer self) scaling))

(defmethod LOCAL-SCALING ((self CONVEX-TRIANGLE-MESH-SHAPE))
  (bullet-physics-c++::CONVEX-TRIANGLE-MESH-SHAPE/GET-LOCAL-SCALING (ff-pointer self)))

(defmethod CALCULATE-PRINCIPAL-AXIS-TRANSFORM ((self CONVEX-TRIANGLE-MESH-SHAPE) (principal TRANSFORM) (inertia VECTOR3) volume)
  (bullet-physics-c++::CONVEX-TRIANGLE-MESH-SHAPE/CALCULATE-PRINCIPAL-AXIS-TRANSFORM (ff-pointer self) principal inertia volume))
...
(defmethod OWNS-BVH ((self BVH-TRIANGLE-MESH-SHAPE))
  (bullet-physics-c++::BVH-TRIANGLE-MESH-SHAPE/GET-OWNS-BVH (ff-pointer self)))

(defmethod PERFORM-RAYCAST ((self BVH-TRIANGLE-MESH-SHAPE) callback (raySource VECTOR3) (rayTarget VECTOR3))
  (bullet-physics-c++::BVH-TRIANGLE-MESH-SHAPE/PERFORM-RAYCAST (ff-pointer self) callback raySource rayTarget))
...
(defmethod PERFORM-CONVEXCAST ((self BVH-TRIANGLE-MESH-SHAPE) callback (boxSource VECTOR3) (boxTarget VECTOR3) (boxMin VECTOR3) (boxMax VECTOR3))
  (bullet-physics-c++::BVH-TRIANGLE-MESH-SHAPE/PERFORM-CONVEXCAST (ff-pointer self) callback boxSource boxTarget boxMin boxMax))

(defmethod PROCESS-ALL-TRIANGLES ((self BVH-TRIANGLE-MESH-SHAPE) callback (aabbMin VECTOR3) (aabbMax VECTOR3))
  (bullet-physics-c++::BVH-TRIANGLE-MESH-SHAPE/PROCESS-ALL-TRIANGLES (ff-pointer self) callback aabbMin aabbMax))

(defmethod REFIT-TREE ((self BVH-TRIANGLE-MESH-SHAPE) (aabbMin VECTOR3) (aabbMax VECTOR3))
  (bullet-physics-c++::BVH-TRIANGLE-MESH-SHAPE/REFIT-TREE (ff-pointer self) aabbMin aabbMax))

(defmethod PARTIAL-REFIT-TREE ((self BVH-TRIANGLE-MESH-SHAPE) (aabbMin VECTOR3) (aabbMax VECTOR3))
  (bullet-physics-c++::BVH-TRIANGLE-MESH-SHAPE/PARTIAL-REFIT-TREE (ff-pointer self) aabbMin aabbMax))

(defmethod NAME ((self BVH-TRIANGLE-MESH-SHAPE))
  (bullet-physics-c++::BVH-TRIANGLE-MESH-SHAPE/GET-NAME (ff-pointer self)))

(defmethod (SETF LOCAL-SCALING) ( (scaling VECTOR3) (self BVH-TRIANGLE-MESH-SHAPE))
  (bullet-physics-c++::BVH-TRIANGLE-MESH-SHAPE/SET-LOCAL-SCALING (ff-pointer self) scaling))

(defmethod OPTIMIZED-BVH ((self BVH-TRIANGLE-MESH-SHAPE))
  (bullet-physics-c++::BVH-TRIANGLE-MESH-SHAPE/GET-OPTIMIZED-BVH (ff-pointer self)))
...
(defmethod (SETF OPTIMIZED-BVH) (
  ...
  (if scaling?
      (progn
        ...
        (bullet-physics-c++::BVH-TRIANGLE-MESH-SHAPE/SET-OPTIMIZED-BVH (ff-pointer self) bvh local-scaling))
      (bullet-physics-c++::BVH-TRIANGLE-MESH-SHAPE/SET-OPTIMIZED-BVH (ff-pointer self) bvh)))

(defmethod BUILD-OPTIMIZED-BVH ((self BVH-TRIANGLE-MESH-SHAPE))
  (bullet-physics-c++::BVH-TRIANGLE-MESH-SHAPE/BUILD-OPTIMIZED-BVH (ff-pointer self)))

(defmethod USES-QUANTIZED-AABB-COMPRESSION ((self BVH-TRIANGLE-MESH-SHAPE))
  (bullet-physics-c++::BVH-TRIANGLE-MESH-SHAPE/USES-QUANTIZED-AABB-COMPRESSION (ff-pointer self)))

(defmethod (SETF TRIANGLE-INFO-MAP) ( triangleInfoMap (self BVH-TRIANGLE-MESH-SHAPE))
  (bullet-physics-c++::BVH-TRIANGLE-MESH-SHAPE/SET-TRIANGLE-INFO-MAP (ff-pointer self) triangleInfoMap))

(defmethod TRIANGLE-INFO-MAP ((self BVH-TRIANGLE-MESH-SHAPE))
  (bullet-physics-c++::BVH-TRIANGLE-MESH-SHAPE/GET-TRIANGLE-INFO-MAP (ff-pointer self)))

(defmethod TRIANGLE-INFO-MAP ((self BVH-TRIANGLE-MESH-SHAPE))
  (bullet-physics-c++::BVH-TRIANGLE-MESH-SHAPE/GET-TRIANGLE-INFO-MAP (ff-pointer self)))

(defmethod CALCULATE-SERIALIZE-BUFFER-SIZE ((self BVH-TRIANGLE-MESH-SHAPE))
  (bullet-physics-c++::BVH-TRIANGLE-MESH-SHAPE/CALCULATE-SERIALIZE-BUFFER-SIZE (ff-pointer self)))

(defmethod ->serial ((self BVH-TRIANGLE-MESH-SHAPE) &key data-buffer serializer &allow-other-keys)
  (bullet-physics-c++::BVH-TRIANGLE-MESH-SHAPE/SERIALIZE (ff-pointer self) data-buffer serializer))

(defmethod ->serial-SINGLE-BVH ((self BVH-TRIANGLE-MESH-SHAPE) serializer)
  (bullet-physics-c++::BVH-TRIANGLE-MESH-SHAPE/SERIALIZE-SINGLE-BVH (ff-pointer self) serializer))

(defmethod ->serial-SINGLE-TRIANGLE-INFO-MAP ((self BVH-TRIANGLE-MESH-SHAPE) serializer)
  (bullet-physics-c++::BVH-TRIANGLE-MESH-SHAPE/SERIALIZE-SINGLE-TRIANGLE-INFO-MAP (ff-pointer self) serializer))
...
(defmethod initialize-instance :after ((obj SCALED-BVH-TRIANGLE-MESH-SHAPE) 
  ...
  (check-type local-Scaling VECTOR3)
  (setf (slot-value obj 'ff-pointer)
        (bullet-physics-c++::MAKE-SCALED-BVH-TRIANGLE-MESH-SHAPE (ff-pointer child-shape)
...
(defmethod AABB+ ((self SCALED-BVH-TRIANGLE-MESH-SHAPE) (t-arg1 TRANSFORM) (aabbMin VECTOR3) (aabbMax VECTOR3))
  (bullet-physics-c++::SCALED-BVH-TRIANGLE-MESH-SHAPE/GET-AABB (ff-pointer self) t-arg1 aabbMin aabbMax))

(defmethod (SETF LOCAL-SCALING) ( (scaling VECTOR3) (self SCALED-BVH-TRIANGLE-MESH-SHAPE))
  (bullet-physics-c++::SCALED-BVH-TRIANGLE-MESH-SHAPE/SET-LOCAL-SCALING (ff-pointer self) scaling))

(defmethod LOCAL-SCALING ((self SCALED-BVH-TRIANGLE-MESH-SHAPE))
  (bullet-physics-c++::SCALED-BVH-TRIANGLE-MESH-SHAPE/GET-LOCAL-SCALING (ff-pointer self)))

(defmethod CALCULATE-LOCAL-INERTIA ((self SCALED-BVH-TRIANGLE-MESH-SHAPE) (mass number) (inertia VECTOR3))
  (bullet-physics-c++::SCALED-BVH-TRIANGLE-MESH-SHAPE/CALCULATE-LOCAL-INERTIA (ff-pointer self) mass inertia))

(defmethod PROCESS-ALL-TRIANGLES ((self SCALED-BVH-TRIANGLE-MESH-SHAPE) callback (aabbMin VECTOR3) (aabbMax VECTOR3))
  (bullet-physics-c++::SCALED-BVH-TRIANGLE-MESH-SHAPE/PROCESS-ALL-TRIANGLES (ff-pointer self) callback aabbMin aabbMax))

(defmethod CHILD-SHAPE ((self SCALED-BVH-TRIANGLE-MESH-SHAPE))
  (bullet-physics-c++::SCALED-BVH-TRIANGLE-MESH-SHAPE/GET-CHILD-SHAPE (ff-pointer self)))

(defmethod CHILD-SHAPE ((self SCALED-BVH-TRIANGLE-MESH-SHAPE))
  (bullet-physics-c++::SCALED-BVH-TRIANGLE-MESH-SHAPE/GET-CHILD-SHAPE (ff-pointer self)))

(defmethod NAME ((self SCALED-BVH-TRIANGLE-MESH-SHAPE))
  (bullet-physics-c++::SCALED-BVH-TRIANGLE-MESH-SHAPE/GET-NAME (ff-pointer self)))

(defmethod CALCULATE-SERIALIZE-BUFFER-SIZE ((self SCALED-BVH-TRIANGLE-MESH-SHAPE))
  (bullet-physics-c++::SCALED-BVH-TRIANGLE-MESH-SHAPE/CALCULATE-SERIALIZE-BUFFER-SIZE (ff-pointer self)))
...
(defmethod LOCAL-SUPPORTING-VERTEX ((self TRIANGLE-MESH-SHAPE) (vec VECTOR3))
  (bullet-physics-c++::TRIANGLE-MESH-SHAPE/LOCAL-GET-SUPPORTING-VERTEX (ff-pointer self) vec))

(defmethod LOCAL-SUPPORTING-VERTEX-WITHOUT-MARGIN ((self TRIANGLE-MESH-SHAPE) (vec VECTOR3))
  (bullet-physics-c++::TRIANGLE-MESH-SHAPE/LOCAL-GET-SUPPORTING-VERTEX-WITHOUT-MARGIN (ff-pointer self) vec))

(defmethod RECALC-LOCAL-AABB ((self TRIANGLE-MESH-SHAPE))
  (bullet-physics-c++::TRIANGLE-MESH-SHAPE/RECALC-LOCAL-AABB (ff-pointer self)))

(defmethod AABB+ ((self TRIANGLE-MESH-SHAPE) (t-arg1 TRANSFORM) (aabbMin VECTOR3) (aabbMax VECTOR3))
  (bullet-physics-c++::TRIANGLE-MESH-SHAPE/GET-AABB (ff-pointer self) t-arg1 aabbMin aabbMax))

(defmethod PROCESS-ALL-TRIANGLES ((self TRIANGLE-MESH-SHAPE) callback (aabbMin VECTOR3) (aabbMax VECTOR3))
  (bullet-physics-c++::TRIANGLE-MESH-SHAPE/PROCESS-ALL-TRIANGLES (ff-pointer self) callback aabbMin aabbMax))

(defmethod CALCULATE-LOCAL-INERTIA ((self TRIANGLE-MESH-SHAPE) (mass number) (inertia VECTOR3))
  (bullet-physics-c++::TRIANGLE-MESH-SHAPE/CALCULATE-LOCAL-INERTIA (ff-pointer self) mass inertia))

(defmethod (SETF LOCAL-SCALING) ( (scaling VECTOR3) (self TRIANGLE-MESH-SHAPE))
  (bullet-physics-c++::TRIANGLE-MESH-SHAPE/SET-LOCAL-SCALING (ff-pointer self) scaling))

(defmethod LOCAL-SCALING ((self TRIANGLE-MESH-SHAPE))
  (bullet-physics-c++::TRIANGLE-MESH-SHAPE/GET-LOCAL-SCALING (ff-pointer self)))

(defmethod MESH-INTERFACE ((self TRIANGLE-MESH-SHAPE))
  (bullet-physics-c++::TRIANGLE-MESH-SHAPE/GET-MESH-INTERFACE (ff-pointer self)))

(defmethod MESH-INTERFACE ((self TRIANGLE-MESH-SHAPE))
  (bullet-physics-c++::TRIANGLE-MESH-SHAPE/GET-MESH-INTERFACE (ff-pointer self)))

(defmethod LOCAL-AABB-MIN ((self TRIANGLE-MESH-SHAPE))
  (bullet-physics-c++::TRIANGLE-MESH-SHAPE/GET-LOCAL-AABB-MIN (ff-pointer self)))

(defmethod LOCAL-AABB-MAX ((self TRIANGLE-MESH-SHAPE))
  (bullet-physics-c++::TRIANGLE-MESH-SHAPE/GET-LOCAL-AABB-MAX (ff-pointer self)))

(defmethod NAME ((self TRIANGLE-MESH-SHAPE))
  (bullet-physics-c++::TRIANGLE-MESH-SHAPE/GET-NAME (ff-pointer self)))
